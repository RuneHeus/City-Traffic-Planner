...
object GeometryUtils:
  ...
  /** Calculates the middle-point M of the line represented by the given 2
    * points
    */
  ...
  def closestMultiple(x: Double, v: Double): Option[Double] =
    val rem = v / x
    val multiple =
      BigDecimal(rem).setScale(0, BigDecimal.RoundingMode.HALF_UP).toDouble
    if v % x == 0 then Some(v) // v = multiple of x
    else if rem % 0.5 == 0 then None // remainder is a.5 => no nearest value
    else Some(multiple * x)

  /** Calculates the offset which needs to get added to a (rotated)
    * RectangleLike after a RectangleLike got resized.
    *
    * @note
    *   All given Point2D's have to be in untransformed coordinates. This
    *   function is an adaption of Christopher Schölzel's equivalent in Java,
    *   Processing.
    * @param cOld
    *   the middle-point of the rectangle BEFORE the resize
    * @param cNew
    *   the middle-point of the rectangle AFTER the resize
    * @param deg
    *   the rotation-degree
    * @param isoCorner
    *   the corner which shouldn't change; in general the opposite corner of the
    *   resized corner
    */
  def calculateRotationOffset(
      cOld: Point2D,
      cNew: Point2D,
      deg: Double,
      isoCorner: Point2D
  ...
  /** Calculates the middle-point between the given 4 points if the 4 points
    * represent a rectangular-bounds.
    */
  def rectangleMiddlePoint(
      p1: Point,
      p2: Point,
      p3: Point,
      p4: Point
  ): Point =
    List(p1, p2, p3, p4).foldLeft((0.0, 0.0)) { case (acc, elem) =>
      acc + elem
    } / (4.0, 4.0)
