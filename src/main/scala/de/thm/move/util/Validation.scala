...
sealed trait Validation[+A, +E]:
  ...
  /** Maps the value with the given function fn, copying the warnings from this
    * Validation to the returning Validation.
    */
  def map[B](fn: A => B): Validation[B, E]

  /** Combines this Validation with the Validation returned from calling fn.
    *
    * fn is called with the value from this Validation. If this Validation and
    * the returning Validation from fn contains warnings they will be joined.
    */
...
case class ValidationSuccess[A](value: A) extends Validation[A, Nothing]:
  ...
  def flatMap[B, EE >: Nothing](fn: A => Validation[B, EE]): Validation[B, EE] =
    fn(value)
  def getValue: A = value
  def getWarnings: List[Nothing] = Nil

/** A validation with warnings. */
case class ValidationWarning[A, E](value: A, warnings: List[E])
    extends Validation[A, E]:
  def map[B](fn: A => B): Validation[B, E] =
    ValidationWarning(fn(value), warnings)
  def flatMap[B, EE >: E](fn: A => Validation[B, EE]): Validation[B, EE] = fn(
    value
  ) match
...
object ValidationWarning:
  ...
  /** Returns a ValidationWarning with the given value a and the given warning w
    * wrapped inside a list.
    */
  def apply[A, E](a: A, w: E): ValidationWarning[A, E] =
    ValidationWarning(a, List(w))
