...
/** Represents shapes with rectangular boundings. For Example Rectangles,
  * Circles, Images
  */
trait RectangleLike:
  self: ResizableShape =>
  ...
  /** Indicates if the coordinates of the shape should get adjusted too, when
    * the width and height get's adjusted.
    *
  ...
  def getBottomLeft: Point =
    (getBoundsInLocal.getMinX, getBoundsInLocal.getMaxY)

  /** Gets the untransformed bottom-right point */
  def getBottomRight: Point =
    (getBoundsInLocal.getMaxX, getBoundsInLocal.getMaxY)
  ...
  topLeftAnchor.setOnMouseDragged(withConsumedEvent { (me: MouseEvent) =>
    ...
    withCheckedBounds(
      getWidth + delta.getX * (-1),
      getHeight + delta.getY * (-1)
    ) {
      if adjustCoordinates then {
        ...
        val deltaP = GeometryUtils.calculateRotationOffset(
          oldMiddle,
          newMiddle,
          getRotate,
          isoCorner
  ...
  /** Calculates the delta-offset for a resize with the value `startMouse` and
    ...
    *
    * @note
    *   To use this function first set startMouse in the XXXMousePressed-handler
    *   and use this function in the XXXMouseDragged-handler. The coordinates of
    *   `startMouse` have to be relative to the scene!
    */
