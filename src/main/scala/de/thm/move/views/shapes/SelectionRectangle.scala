...
/** A rectangle that highlights the underlying shape by drawing a dotted-line
  * around the underlying shape.
  */
class SelectionRectangle(selectedShape: Node) extends Rectangle:
  ...
  val widthProp = new SimpleDoubleProperty(
    selectedShape.getBoundsInLocal().getWidth
  )
  val heightProp = new SimpleDoubleProperty(
    selectedShape.getBoundsInLocal().getHeight
  )

  selectedShape.boundsInParentProperty().addListener {
    (_: Bounds, newBounds: Bounds) =>
  ...
  this
    .heightProperty()
    .bind(heightProp.add(distanceToShape).add(aditionalSpace))
