...
/** Turns a Node into a rotatable node by adding anchors for rotation and rotate
...
trait RotatableShape:
  ...
  private def setupListener(anchor: Anchor): Unit =
    ...
    anchor.setOnMouseDragged(withConsumedEvent { (me: MouseEvent) =>
      ...
      val vector1 = GeometryUtils.vectorOf(
        middlePoint,
        (translatedStart.getX, translatedStart.getY)
      )
      val vector2 = GeometryUtils.vectorOf(
        middlePoint,
        (translatedEnd.getX, translatedEnd.getY)
      )
      val scalar =
        GeometryUtils.scalarProduct(vector1, vector2) // scalarproduct
      val lengthVector1 =
        GeometryUtils.vectorLength(vector1) // norm/length vector1
      val lengthVector2 =
        GeometryUtils.vectorLength(vector2) // norm/length vector2
      ...
      val angle =
        ...
        then scala.math.acos(cos_angle)
        else if translatedEnd.getY > translatedStart.getY && (anchor == topLeftAnchor || anchor == bottomLeftAnchor)
        then scala.math.acos(cos_angle) * (-1)
        else if anchor == topLeftAnchor || anchor == bottomLeftAnchor then
          scala.math.acos(cos_angle)
        else scala.math.acos(cos_angle) * (-1)
