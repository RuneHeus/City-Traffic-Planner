...
object Dialogs:
  ...
  val moFileFilter =
    new FileChooser.ExtensionFilter("Modelica files (*.mo)", "*.mo")
  val jsonFileFilter =
  ...
  def newScaleDialog()(implicit
      marshaller: StringMarshaller[Int]
  ): InputDialog[Int] =
    val dialog = new InputDialog(
      fontBundle.getString("scaledialog.inputfield") -> Some(1)
    )
  ...
  def newPaperSizeDialog(width: Double, height: Double)(implicit
      marshaller: StringMarshaller[Double]
  ): InputDialog[Double] =
    val dialog = new InputDialog(
      fontBundle.getString("papersizedialog.inputfield-width") -> Some(width),
      fontBundle.getString("papersizedialog.inputfield-height") -> Some(height)
    )
  ...
  def newGridSizeDialog(
      cellSize: Int
  )(implicit marshaller: StringMarshaller[Int]): InputDialog[Int] =
    val dialog = new InputDialog(
      fontBundle.getString("gridsizedialog.inputfield") -> Some(cellSize)
    )
  ...
  def newSvgFileChooser(): FileChooser =
    newFileChooser(svgFileFilter)(allFilesFilter, svgFileFilter)
  def newPngFileChooser(): FileChooser =
    newFileChooser(pngFileFilter)(allFilesFilter, pngFileFilter)
  ...
  def newConfirmationDialog(additionalInfo: String = ""): Alert =
    ...
    alert.setHeaderText(
      fontBundle.getString("alert.confirmation.header") + s"\n$additionalInfo"
    )
    alert
