...
/** A dialog that asks the user for values for the given `names` and converts
  * user's input using the implicitly given Marshaller. The marshaller marshals
  * `String => A` and `A => String`.
  *
  * The result is a dialog which returns a `List[A]`.
  */
class InputDialog[A: StringMarshaller](names: (String, Option[A])*)
    extends Dialog[List[A]]:
  // setup css
  ...
  private val btns = List(
    new ButtonType(Global.fontBundle.getString("ok-btn"), ButtonData.OK_DONE),
    new ButtonType(
      Global.fontBundle.getString("cancel-btn"),
  ...
  for ((lbl, txtField), idx) <- inputList.zipWithIndex do
    pane.add(lbl, 0, idx)
...
object InputDialog:
  def fromPairs[A: StringMarshaller](
      names: (String, Option[A])*
  ): InputDialog[A] = new InputDialog(names: _*)
