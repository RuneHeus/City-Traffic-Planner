...
class ExternalChangesDialog(filename: String)
    extends Alert(AlertType.CONFIRMATION):
  val overwriteAnnotationsBtn = new ButtonType(
    Global.fontBundle.getString("filechanges.overwrite-btn")
  )
  ...
  setHeaderText(
    Global.fontBundle.getString("filechanges.header").replace("{x}", filename)
  )
