...
/** Represents elements that can calculate a distance for a snapping-mechanism
  * to a specific element.
  *
  * E.G.: A grid with lines on which elements can get snapped (a.k.a.
  * snap-to-grid).
  */
trait SnapLike:
...
object SnapLike:
  ...
  def applySnapToGrid(
      snaplike: SnapLike,
      node: Node with MovableShape
  ): Unit =
    val delta = getSnapToGridDistance(
      snaplike,
  ...
  def getSnapToGridDistance(snaplike: SnapLike, x: Double, y: Double): Point =
    val deltaX =
      snaplike.getClosestXPosition(x).map(_.toDouble - x).getOrElse(0.0)

    val deltaY =
      snaplike.getClosestYPosition(y).map(_.toDouble - y).getOrElse(0.0)
