...
/** Implements the different ways to fill a shape: a solid colour, a gradient, a
  * pattern, etc.
  */
object FillPattern extends Enumeration:
  type FillPattern = Value
  val None, Solid, HorizontalCylinder, VerticalCylinder, Sphere, Horizontal,
      Vertical, Cross, Forward, Backward, CrossDiag = Value
  ...
  def getFillColor(
    ...
    /** Implementation Nodes: LinearGradients - Horizontal: create gradient from
      * (bottom to middle); reflect them on (middle to top) LinearGradients -
      * Vertical: create gradient from (left to middle); reflect them on (middle
      * to right)
      */
  ...
  private def withCanvas(width: Double, height: Double, fillC: Color)(
      fn: CustomCanvas => Unit
  ): Paint =
  ...
  /** A canvas for drawing FillPatterns.
    */
  class CustomCanvas(width: Double, height: Double, backgroundColor: Color)
      extends Canvas(width, height):
    val distance = config
    ...
    /** Draws the FillPattern.Horizontal structure from modelica onto this
