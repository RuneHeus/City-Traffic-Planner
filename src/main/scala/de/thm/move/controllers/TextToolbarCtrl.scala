...
/** Controller for the TopTextToolbar.
  *
  * Please be aware that a correct SelectedShapeCtrl is set after creating this
  * controller and before it's first use! Invoke in parent's
  * Controller#initizalize() method the TextToolbarCtrl#setSelectedShapeCtrl()!
  */
class TextToolbarCtrl extends Initializable:
  ...
  override def initialize(location: URL, resources: ResourceBundle): Unit =
    ...
    val fontColor =
      ResourceUtils.asColor("colorChooser.strokeColor").getOrElse(Color.BLACK)
    ...
    val alignmentToggles =
      alignmentGroup.getToggles().asScala.map(_.asInstanceOf[ToggleButton])
    alignmentToggles foreach { x =>
      x.setOnAction { (ae: ActionEvent) =>
        ae.getSource match
          case x: ToggleButton =>
            selectedShapeCtrl.setTextAlignment(alignmentById(x.getId))
          case _ => // ignore
      }
    }

  /** Sets the controller for selected elements. Due to the fact, that the
    * FXMLLoader can't inject object-attributes we need a way to add fields
