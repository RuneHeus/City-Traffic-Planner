...
class ContextMenuCtrl(drawPanel: DrawPanel, changeLike: ChangeDrawPanelLike):
  ...
  private def newContextMenu(
      underlyingElement: ResizableShape
  ): ShapeContextMenu =
    ...
    menu.duplicateElementItem.setOnAction(
      onDuplicateElementPressed(underlyingElement) _
    )
    menu.resetRotationElementItem.setOnAction(
      onResetRotationElementPressed(underlyingElement) _
    )
    menu.rotate90ClockwiseItem.setOnAction(
      onRotateByDegreePressed(underlyingElement, 90) _
    )
    menu.rotate90CounterClockwiseItem.setOnAction(
      onRotateByDegreePressed(underlyingElement, -90) _
    )
    menu.rotate45ClockwiseItem.setOnAction(
      onRotateByDegreePressed(underlyingElement, 45) _
    )
    menu.rotate45CounterClockwiseItem.setOnAction(
      onRotateByDegreePressed(underlyingElement, -45) _
    )
    ...
    underlyingElement match
      ...
      case img: ResizableImage if img.srcEither.isLeft =>
        val encodeBase64Item = new MenuItem(
          fontBundle.getString("context.encode-base64")
        )
  ...
  def setupContextMenu(node: ResizableShape): Unit =
    ...
    node.setOnMouseClicked { (me: MouseEvent) =>
      if me.getButton == MouseButton.SECONDARY then
        contextMenu.show(node, me.getScreenX, me.getScreenY)
    }

  private def onForegroundPressed(
      underlyingElement: Node
  )(ae: ActionEvent): Unit =
  ...
  private def onBackgroundPressed(
      underlyingElement: Node
  )(ae: ActionEvent): Unit =
    val oldIdx = drawPanel.getChildren.indexOf(underlyingElement)
    history.execute {
  ...
  private def onBecierPressed(
      polygon: QuadCurveTransformable
  )(ae: ActionEvent): Unit =
  ...
  private def onUnsmoothPressed(
      curved: AbstractQuadCurveShape
  )(ae: ActionEvent): Unit =
  ...
  def onResetRotationElementPressed(
      shape: ResizableShape
  )(ae: ActionEvent): Unit =
  ...
  def onEncodePressed(resImg: ResizableImage)(ae: ActionEvent): Unit =
    resImg.srcEither match
      case Left(uri) =>
  ...
  def onRotateByDegreePressed(shape: RotatableShape, degree: Double)(
      ae: ActionEvent
  ): Unit =
    shape.rotate(degree)
