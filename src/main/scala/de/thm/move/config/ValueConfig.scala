...
class ValueConfig(url: URL):
  ...
  def getConvertedValues[A](implicit ev: Convertable[String, A]): List[A] =
    getValues.map(ev.convert)
  def setUniqueValue(v: String) = if !values.contains(v) then setValue(v)
  ...
  def saveConfig(): Unit =
    val writer =
      Files.newBufferedWriter(Paths.get(url.toURI), Charset.forName("UTF-8"))
