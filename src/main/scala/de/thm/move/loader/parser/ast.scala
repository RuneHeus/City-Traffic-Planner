...
object ast:
  ...
  case class IconElements(
      coordinationSystem: Option[CoordinateSystem],
  ...
  sealed trait ShapeElement extends ModelicaAst:
    ...
    /** A list of possible warnings that occured during parsing the file. Will
      * be used to warn the user when some Property can't get used
      */
    var warnings: List[String] = Nil

  case class GraphicItem(
      visible: Boolean = true,
      origin: Point = (0, 0),
      rotation: Double = 0.0
  )
  case class FilledShape(
  ...
  abstract class AbstractImage(gItem: GraphicItem, extent: Extent)
      extends ShapeElement
  case class ImageURI(gItem: GraphicItem, extent: Extent, fileName: String)
