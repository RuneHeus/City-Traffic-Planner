...
class ParserAndConvertTest extends MoveSpec:
  ...
  "Parsing and Converting" should "parse & convert Rectangles" in {
    ...
    val conv = new ShapeConverter(
      1,
      ShapeConverter.gettCoordinateSystemSizes(parsed),
      null
    )
    val convertedRectangle = conv
      .getShapes(parsed)
      .head
    ...
    val conv3 = new ShapeConverter(
      1,
      ShapeConverter.gettCoordinateSystemSizes(parsed),
      null
    )
    val convertedRectangle3 = conv
      .getShapes(parsed2)
      .head
      .asInstanceOf[(ResizableRectangle, Option[String])]
      ._1
